...
import { PresetColor } from 'src/types/config';
import { useDispatch, useSelector } from 'src/store';
import { onChangePresetColor } from 'src/store/reducers/config';
...
const ColorScheme = () => {
  ...
  const { mode, presetColor } = useSelector(state => state.config);
  ...
  const handlePresetColorChange = (event: ChangeEvent<HTMLInputElement>) => {
    dispatch(onChangePresetColor(event.target.value as PresetColor));
  };
